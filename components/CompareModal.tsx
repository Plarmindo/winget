             ...
             parsedData ? (
                 <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                     ...
                     <div className="border border-[var(--app-border)] rounded-xl overflow-hidden shadow-lg">
                        <div className="grid bg-[var(--app-surface)] border-b border-[var(--app-border)]" style={{ gridTemplateColumns: `150px repeat(${parsedData.apps.length}, 1fr)` }}>
                            <div className="p-4 font-bold text-[var(--app-text-muted)] flex items-center">Feature</div>
                            {parsedData.apps.map((app, i) => (
                                <div key={i} className="p-4 font-bold text-[var(--app-text)] text-center border-l border-[var(--app-border)] bg-[var(--app-primary)]/10">{app}</div>
                            ))}
                        </div>
                        {parsedData.features.map((feature, idx) => (
                             <div key={idx} className={`grid hover:bg-[var(--app-surface)]/50 transition-colors ${idx !== parsedData.features.length - 1 ? 'border-b border-[var(--app-border)]' : ''}`} style={{ gridTemplateColumns: `150px repeat(${parsedData.apps.length}, 1fr)` }}>
                                 <div className="p-3 text-sm font-medium text-[var(--app-text-muted)] bg-[var(--app-bg)]/30">{feature.name}</div>
                                 {feature.values.map((val, vIdx) => (
                                     <div key={vIdx} className="p-3 text-sm text-center border-l border-[var(--app-border)] flex items-center justify-center">{val}</div>
                                 ))}
                             </div>
                        ))}
                     </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid gap-4">
                                {parsedData.pros.map((p, i) => (
                                    <div key={i} className="bg-[var(--app-surface)] rounded-xl p-4 border border-emerald-900/30 shadow-sm relative overflow-hidden group">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid gap-4">
                                {parsedData.cons.map((c, i) => (
                                    <div key={i} className="bg-[var(--app-surface)] rounded-xl p-4 border border-red-900/30 shadow-sm relative overflow-hidden group">
