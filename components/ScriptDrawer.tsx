...
import { X, Copy, Trash2, Terminal, CheckCircle2, CheckSquare, Square, Info, AlertTriangle, ArrowDownUp, Search, RefreshCw, ShieldAlert, Database, Check, PieChart, Layers, Download, Loader2, RotateCcw } from 'lucide-react';
...
export const ScriptDrawer: React.FC<ScriptDrawerProps> = ({ isOpen, onClose, cart, onRemove, onClear, mode, onSwitchToUpgrade, onDeepScan }) => {
  ...
  // Confirmation state for uninstall (embedded in modal now)
  const [selectedConfirmPhrase, setSelectedConfirmPhrase] = useState(UNINSTALL_CONFIRMATION_PHRASES[0]);

  // Sync selectedIds with cart changes (default to all selected when cart changes)
  useEffect(() => {
    setSelectedIds(cart.map(p => p.id));
  }, [cart]);

  // Reset states when closed or mode changes
  useEffect(() => {
  ...
  const getFilteredAndSortedCart = () => {
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === 'name') {
        ...
        // Simple numeric/semver sort using localeCompare with numeric option
        valA = a.version || '0';
        valB = b.version || '0';
        return sortAsc 
  ...
  // Logic to calculate excluded packages for warning
  ...
  const generateScript = () => {
    ...
    // Advanced Uninstall Script Generation
    if (mode === 'uninstall') {
...
#   
#   --id : Targets the specific application via its unique identifier.
#   -e   : Enforces 'exact' matching. This prevents accidental uninstallation of 
#          packages with similar names (e.g., removing 'Java' instead of 'JavaScript').
#
...
foreach ($id in $packages) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "ERROR: Failed to uninstall $id. Exit Code: $LASTEXITCODE" -ForegroundColor Red
        Write-Host "Possible reasons:" -ForegroundColor Gray
        Write-Host "  1. Not running as Administrator." -ForegroundColor Gray
        Write-Host "  2. Package not found or already removed." -ForegroundColor Gray
        Write-Host "  3. Interactive prompt required (try running manually)." -ForegroundColor Gray
    }
}
Write-Host "--------------------------------"
...
# Packages without available updates are excluded from this script.`;
        // Exclude packages that are already up to date
    ...
    // Install Script Generation (Smart)
    if (mode === 'install') {
        const upgrades = activePackages.filter(p => p.availableVersion);
        const installs = activePackages.filter(p => !p.availableVersion);
  ...
  // Calculate selection state for current view
  const visibleIds = filteredCart.map(p => p.id);
  const allVisibleSelected = visibleIds.length > 0 && visibleIds.every(id => selectedIds.includes(id));
  ...
  const getActionLabel = () => {
      ...
      // More visible "Install All" label logic
            ...
            <>
              ...
              {!showPreview && !isGenerating && (
                ...
                <div className={`p-3 rounded-lg border bg-[var(--app-surface)]/50 flex flex-col gap-2 ${
                  ...
                  {/* Deep Scan / Check for Updates Action */}
              ...
              {/* Selected List (Hidden during Generation/Preview) */}
              {!showPreview && !isGenerating && (
              ...
              {showConfirmModal && (
                <div className="absolute inset-0 bg-[var(--app-bg)]/95 backdrop-blur-sm z-20 p-6 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-200">
                   {mode === 'uninstall' ? (
                     // Uninstall Specific Confirmation
                     <>
