...
export const DataTab: React.FC<DataTabProps> = ({ onClearData }) => {
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
       <div>
          <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Database size={20} /> Backup & Restore</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <button onClick={handleExportData} className="group p-6 bg-[var(--app-bg)] hover:bg-[var(--app-surface)] border border-[var(--app-border)] rounded-2xl flex flex-col items-center justify-center gap-4 text-[var(--app-text)] transition-all hover:border-[var(--app-primary)] hover:shadow-lg">
          ...
          <div className="bg-red-500/5 border border-red-500/20 rounded-2xl overflow-hidden">
              <div className="divide-y divide-red-500/10">
                  <div className="p-4 flex items-center justify-between">
                      ...
                      <button onClick={() => onClearData('cart')} className="px-4 py-2 bg-[var(--app-surface)] hover:bg-red-500 hover:text-white text-[var(--app-text)] border border-[var(--app-border)] hover:border-red-500 rounded-lg text-xs font-bold transition-all">Clear Cart</button>
                  </div>
                  <div className="p-4 flex items-center justify-between">
                      <div>
                      ...
                      <button onClick={() => onClearData('chat')} className="px-4 py-2 bg-[var(--app-surface)] hover:bg-red-500 hover:text-white text-[var(--app-text)] border border-[var(--app-border)] hover:border-red-500 rounded-lg text-xs font-bold transition-all">Clear Chat</button>
                  </div>
                   <div className="p-4 flex items-center justify-between bg-red-500/10">
                      ...
                      <button onClick={() => onClearData('all')} className="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-xs font-bold transition-all shadow-lg shadow-red-900/20">Reset App</button>
                  </div>
              </div>
          </div>
       </div>
