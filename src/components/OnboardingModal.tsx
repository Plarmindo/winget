...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({ onClose }) => {
    ...
    return (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className={`relative bg-[var(--app-surface)] text-[var(--app-text)] w-full max-w-lg rounded-3xl shadow-2xl border border-[var(--app-border)] overflow-hidden transition-all duration-500 transform ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-8'}`}>
