    ...
    >
      ...
      <div className="p-3 bg-[var(--app-surface)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-bold ${isActive ? 'text-[var(--app-primary)]' : 'text-[var(--app-text)]'}`}>
            {theme.name}
          </span>
        </div>
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className="p-1.5 hover:bg-[var(--app-bg)] rounded-md text-[var(--app-text-muted)] hover:text-[var(--app-text)]"
            title="Edit/Copy"
          >
          ...
          {theme.isCustom && onDelete && (
            <button
              ...
              className="p-1.5 hover:bg-red-500/10 rounded-md text-[var(--app-text-muted)] hover:text-red-500"
              title="Delete"
            >
              <Trash2 size={12} />
            </button>
...
export const AppearanceTab: React.FC<AppearanceTabProps> = ({ settings, onUpdateSettings }) => {
  ...
  const itemsPerPage =
    typeof settings.itemsPerPage === 'number' && settings.itemsPerPage > 0 ? settings.itemsPerPage : 9;
  const currentThemes = settings.themes || DEFAULT_THEMES;
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {!editingThemeId && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[var(--app-primary)] hover:opacity-90 text-white rounded-lg text-sm font-bold shadow-md transition-all"
            >
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg border border-[var(--app-border)] hover:bg-[var(--app-surface)] text-sm font-medium transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSaveTheme}
                  className="flex items-center gap-2 px-4 py-1.5 bg-green-600 hover:bg-green-500 text-white rounded-lg text-sm font-bold transition-colors shadow-lg shadow-green-900/20"
                >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => editedTheme && setEditedTheme({ ...editedTheme, name: e.target.value })}
                    className="w-full bg-[var(--app-surface)] border border-[var(--app-border)] rounded-lg px-3 py-2 text-sm text-[var(--app-text)] focus:ring-2 focus:ring-[var(--app-primary)] focus:border-transparent outline-none"
                  />
                </div>
                <div className="p-4 bg-[var(--app-surface)] rounded-xl border border-[var(--app-border)]">
                  <p className="text-xs font-bold text-[var(--app-text-muted)] uppercase tracking-wider mb-3">
                    Live Preview
                  </p>
                  <div className="flex gap-2">
                    <div
                      className="h-20 w-full rounded-lg shadow-inner"
                      style={{ backgroundColor: editedTheme.colors?.bg }}
                    ></div>
                    <div
                      className="h-20 w-full rounded-lg shadow-lg border border-white/10"
                      style={{ backgroundColor: editedTheme.colors?.surface }}
                    ></div>
                    <div
                      className="h-20 w-full rounded-lg shadow-lg flex items-center justify-center text-white font-bold"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {editedTheme.colors &&
                  Object.entries(editedTheme.colors).map(([key, val]) => (
                    <div
                      key={key}
                      className="bg-[var(--app-surface)] p-2 rounded-lg border border-[var(--app-border)] flex items-center gap-3"
                    >
                      <div className="relative w-10 h-10 rounded-lg overflow-hidden border border-[var(--app-border)] shadow-sm flex-shrink-0 group">
                        <input
                          ...
                          onChange={(e) =>
                            editedTheme &&
                            setEditedTheme({ ...editedTheme, colors: { ...editedTheme.colors, [key]: e.target.value } })
                          }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 bg-[var(--app-bg)]/50 rounded-xl border border-[var(--app-border)] flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-sm">Items Per Page</p>
                <p className="text-xs text-[var(--app-text-muted)]">Cards per grid view</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(val) && val >= 0) onUpdateSettings({ ...settings, itemsPerPage: val });
                }}
                className="w-16 bg-[var(--app-surface)] border border-[var(--app-border)] text-[var(--app-text)] rounded-lg px-2 py-1 text-sm text-center focus:outline-none focus:ring-1 focus:ring-[var(--app-primary)]"
              />
            </div>
          </div>
          <div
            className="p-4 bg-[var(--app-bg)]/50 rounded-xl border border-[var(--app-border)] flex items-center justify-between cursor-pointer hover:bg-[var(--app-bg)] transition-colors"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--app-text-muted)]">Reduce padding size</p>
              </div>
            </div>
            <button
              className={`w-10 h-5 rounded-full transition-colors relative ${settings.compactMode ? 'bg-[var(--app-primary)]' : 'bg-[var(--app-border)]'}`}
            >
              <div
                className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${settings.compactMode ? 'translate-x-5' : 'translate-x-0'}`}
              />
            </button>
          </div>
          <div
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--app-text-muted)]">Disable smooth animations</p>
              </div>
            </div>
            <button
              className={`w-10 h-5 rounded-full transition-colors relative ${settings.reducedMotion ? 'bg-[var(--app-primary)]' : 'bg-[var(--app-border)]'}`}
            >
              <div
                className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${settings.reducedMotion ? 'translate-x-5' : 'translate-x-0'}`}
              />
            </button>
          </div>
        </div>
