...
import { Palette, Plus, Edit2, Copy, Trash2, Save, Grid, Minus, Activity } from 'lucide-react';
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="space-y-4">
                     <div>
                        ...
                        <input type="text" value={editedTheme.name} onChange={(e) => setEditedTheme({ ...editedTheme, name: e.target.value })} className="w-full bg-[var(--app-surface)] border border-[var(--app-border)] rounded-lg px-3 py-2 text-sm text-[var(--app-text)] focus:ring-2 focus:ring-[var(--app-primary)] focus:border-transparent outline-none" />
                     </div>
                     ...
                     {editedTheme.colors && Object.entries(editedTheme.colors).map(([key, val]) => (
                        <div key={key} className="bg-[var(--app-surface)] p-2 rounded-lg border border-[var(--app-border)] flex items-center gap-3">
                           <div className="relative w-10 h-10 rounded-lg overflow-hidden border border-[var(--app-border)] shadow-sm flex-shrink-0 group">
                             <input type="color" value={val} onChange={(e) => setEditedTheme({ ...editedTheme, colors: { ...editedTheme.colors, [key]: e.target.value } })} className="absolute -top-2 -left-2 w-16 h-16 cursor-pointer border-none p-0 opacity-0" />
