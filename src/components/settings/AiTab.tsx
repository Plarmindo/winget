...
export const AiTab: React.FC<AiTabProps> = ({ settings, onUpdateSettings }) => {
  ...
  const hasErrors = Object.keys(validationErrors).length > 0 || !localAiConfig.modelId || (localAiConfig.provider !== 'gemini' && localAiConfig.provider !== 'ollama' && !localAiConfig.baseUrl);
