...
export const ConnectionsTab: React.FC<ConnectionsTabProps> = ({ settings, onUpdateSettings }) => {
   ...
   const handleSave = async () => {
      ...
      if (isValid) {
         setStatus('success');
         onUpdateSettings({ ...settings, githubToken: token });
   ...
   return (
      <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
         <div>
            <h3 className="text-xl font-bold mb-1 flex items-center gap-2"><Github size={24} /> GitHub Integration</h3>
            <p className="text-sm text-[var(--app-text-muted)]">Connect your account to search and clone repositories directly.</p>
         </div>

         <div className="bg-[var(--app-bg)]/50 border border-[var(--app-border)] rounded-xl p-6 space-y-4">
            <div>
               <label className="text-xs font-bold text-[var(--app-text-muted)] uppercase tracking-wider block mb-2">Personal Access Token (PAT)</label>
               <div className="flex gap-2">
                  ...
                  <button
                     ...
                     className={`px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 transition-all ${status === 'success' ? 'bg-green-600 text-white' :
                  ...
                  >
                     ...
                     {status === 'idle' ? 'Verify & Save' : status === 'success' ? 'Saved' : status === 'error' ? 'Invalid' : 'Checking'}
                  </button>
               </div>
               {settings.githubToken && (
                  <div className="mt-2 flex items-center gap-2 text-xs text-green-500 bg-green-500/10 px-2 py-1 rounded w-fit">
                     <Check size={12} />
                     <span>Token saved (ends in {settings.githubToken.slice(-4)})</span>
                     <button
                        onClick={() => {
                           setToken('');
               ...
               <p className="text-xs text-[var(--app-text-muted)] mt-2">
                  Generate a token in GitHub Settings &gt; Developer Settings &gt; Personal Access Tokens (Classic).
                  Scopes needed: <code>repo</code> (for private repos) or none (public only).
               </p>
            </div>
         </div>
      </div>
