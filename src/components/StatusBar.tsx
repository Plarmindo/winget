        ...
        <>
            ...
            {isOpen && (
                <div className="fixed bottom-8 left-0 right-0 h-64 bg-[#0d1117] border-t border-[var(--app-border)] z-40 p-4 overflow-y-auto font-mono text-xs shadow-inner animate-in slide-in-from-bottom-2 text-gray-300">
                    <div className="flex justify-between items-center mb-2 sticky top-0 bg-[#0d1117] pb-2 border-b border-gray-800">
                        ...
                        <button onClick={() => setLogs([])} className="text-[10px] px-2 py-1 bg-gray-800 rounded hover:bg-gray-700 transition-colors">Clear</button>
                    </div>
                    {logs.map((log, i) => (
                        <div key={i} className="flex gap-2 py-0.5 border-b border-gray-800/50 last:border-0 hover:bg-white/5 px-1 rounded">
                            <span className={`w-20 shrink-0 font-bold ${log.operation === 'install' ? 'text-green-400' : log.operation === 'uninstall' ? 'text-red-400' : 'text-blue-400'}`}>[{log.operation}]</span>
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-[var(--app-surface)] border-t border-[var(--app-border)] h-8 flex items-center px-4 justify-between z-50 text-xs shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
                <div className="flex items-center gap-3 overflow-hidden flex-1 mr-4">
                    {currentEvent ? (
                        <>
                            ...
                            <span className="font-semibold text-[var(--app-text)] shrink-0">{currentEvent.operation.toUpperCase()}: {currentEvent.package}</span>
                            <span className="text-[var(--app-text-muted)] truncate hidden md:inline-block opacity-70"> - {currentEvent.message}</span>
                        </>
                    ) : logs.length > 0 ? (
                        <span className="text-[var(--app-text-muted)]">Idle. Last operation: {logs[logs.length - 1].operation}</span>
                    ) : (
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={() => { setIsVisible(false); setIsOpen(false); }} className="p-1 hover:bg-[var(--app-bg)] rounded text-[var(--app-text-muted)] hover:text-red-500" title="Close Status Bar">
                        <X size={12} />
                    </button>
                </div>
            </div>
