...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ handleSearch }) => {
            ...
            <div className="flex justify-center gap-3 flex-wrap">
               ...
               {isAdding ? (
                  <div className="flex items-center gap-2 animate-in fade-in zoom-in duration-200">
                     ...
                     />
                     <button onClick={() => handleAdd(newSubject)} className="p-2 bg-[var(--app-primary)] text-white rounded-full hover:opacity-90"><Plus size={14} /></button>
                     <button onClick={() => setIsAdding(false)} className="p-2 text-[var(--app-text-muted)] hover:text-[var(--app-text)]"><X size={14} /></button>
                  </div>
               ) : (
                  <button onClick={() => setIsAdding(true)} className="px-4 py-2 rounded-full text-sm border border-dashed border-[var(--app-border)] text-[var(--app-text-muted)] hover:text-[var(--app-primary)] hover:border-[var(--app-primary)] flex items-center gap-2">
                     <Plus size={14} /> Add
                  </button>
               )}
