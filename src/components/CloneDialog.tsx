...
export const CloneDialog: React.FC<CloneDialogProps> = ({ isOpen, onClose, onConfirm, repoName, repoUrl }) => {
  ...
  const handleBrowse = async () => {
    if (!isTauri()) {
      showToast('File picker is only available in desktop mode', 'warning');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--app-text)] mb-2">Repository URL</label>
          ...
          {/* Destination Path */}
          <div>
            <label className="block text-sm font-medium text-[var(--app-text)] mb-2">Destination Directory</label>
          ...
          <div className="flex items-center gap-2 p-3 bg-[var(--app-bg)] border border-[var(--app-border)] rounded-lg">
            ...
            />
            <label
              htmlFor="remember-location"
              className="text-sm text-[var(--app-text)] cursor-pointer flex items-center gap-2"
            >
              <Clock size={14} />
