    ...
    <span>
      {parts.map((part, i) =>
        part.toLowerCase() === safeHighlight.toLowerCase() ? (
          <span key={i} className="text-[var(--app-primary)] font-bold">
            {part}
          </span>
        ) : (
...
export const SearchInput: React.FC<SearchInputProps> = ({
  ...
  const getSuggestions = () => {
    ...
    const popularMatches = POPULAR_SUGGESTIONS.filter(
      (item) =>
        item.toLowerCase().includes(normalizedValue) && !history.some((h) => h.toLowerCase() === item.toLowerCase())
    )
      .slice(0, 8)
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <div className="relative group">
        ...
        />
        <Search
          className="absolute left-4 top-2.5 text-[var(--app-text-muted)] group-focus-within:text-[var(--app-primary)] transition-colors pointer-events-none"
          size={18}
        />
      ...
      {showDropdown && (
        <div
          ...
          role="listbox"
          className="absolute top-full left-0 right-0 mt-2 bg-[var(--app-surface)] border border-[var(--app-border)] rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100 max-h-[400px] overflow-y-auto"
        >
