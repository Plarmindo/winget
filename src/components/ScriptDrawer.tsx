...
import { X, Terminal, CheckSquare, Square, Trash2, Play, Download } from 'lucide-react';
...
export const ScriptDrawer: React.FC<ScriptDrawerProps> = ({ isOpen, onClose }) => {
  ...
  const handleExecuteNow = async () => {
    ...
    if (packageManager === 'github') {
      showToast(
        'GitHub repositories cannot be batch installed. Please install them individually from the search results.',
        'warning'
      );
      return;
    }
    ...
    const confirmed = await confirmDialog({
      message: `${mode} ${selectedIds.length} packages?`,
    ...
    for (let i = 0; i < ops.length; i++) {
      ...
      setBatchOperations((prev) => prev.map((op, idx) => (idx === i ? { ...op, status: 'processing' } : op)));

      try {
        await executeRealCommand(packageManager, mode, [ops[i].id]);

        // Mark as success
        setBatchOperations((prev) => prev.map((op, idx) => (idx === i ? { ...op, status: 'success' } : op)));
        ...
        if (e && typeof e === 'object' && 'code' in e && (e as { code?: unknown }).code === 'USER_CANCELLED') {
          setBatchOperations((prev) => prev.map((op, idx) => (idx === i ? { ...op, status: 'pending' } : op)));
          continue;
        }

        // Mark as error
        setBatchOperations((prev) =>
          prev.map((op, idx) => (idx === i ? { ...op, status: 'error', errorMessage: errMsg } : op))
        );
  ...
  const handleRetryFailed = async () => {
    ...
    setBatchOperations((prev) =>
      prev.map((op) => (op.status === 'error' ? { ...op, status: 'pending', errorMessage: undefined } : op))
    );
    setIsCancelled(false);
    ...
    for (let i = 0; i < batchOperations.length; i++) {
      ...
      try {
        ...
        if (e && typeof e === 'object' && 'code' in e && (e as { code?: unknown }).code === 'USER_CANCELLED') continue;
        const errMsg = e instanceof Error ? e.message : String(e);
        setBatchOperations((prev) =>
          prev.map((op, idx) => (idx === i ? { ...op, status: 'error', errorMessage: errMsg } : op))
        );
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-full sm:w-[500px] bg-[var(--app-surface)] border-l border-[var(--app-border)] z-50 transform transition-transform duration-300 shadow-2xl flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex items-center justify-between p-5 border-b border-[var(--app-border)] bg-[var(--app-surface)]">
          <div className="flex items-center space-x-3">
            <div className={`p-2 rounded-lg bg-[var(--app-primary)]/20 text-[var(--app-primary)]`}>
              <Terminal size={20} />
            </div>
            <div>
          ...
          <button onClick={onClose} className="p-2 hover:bg-[var(--app-bg)] rounded-lg">
            <X size={20} />
          </button>
        </div>
          ...
          {cart.length === 0 ? (
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-[var(--app-surface)] p-2 rounded border border-[var(--app-border)]">
                <button onClick={toggleAll} className="text-xs flex items-center gap-2 font-bold uppercase">
                  {selectedIds.length === cart.length ? <CheckSquare size={16} /> : <Square size={16} />} Select All
                </button>
              ...
              <div className="space-y-2">
                {cart.map((pkg) => (
                  <div
                    key={pkg.id}
                    className="flex items-center justify-between p-2 rounded bg-[var(--app-surface)] border border-[var(--app-border)]"
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      <button
                        onClick={() => toggleSelection(pkg.id)}
                        className={selectedIds.includes(pkg.id) ? 'text-[var(--app-primary)]' : 'text-gray-500'}
                      >
                        {selectedIds.includes(pkg.id) ? <CheckSquare size={16} /> : <Square size={16} />}
                      </button>
                      <span className="truncate text-sm font-medium">{pkg.name}</span>
                    </div>
                    <button onClick={() => removeFromCart(pkg.id)} className="text-gray-500 hover:text-red-400">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
            ...
            <ScriptPreview
              ...
              analysis={{ total: activePackages.length, upgrades: 0, installs: activePackages.length }}
              excludedCount={0}
          ...
          <div className="p-5 border-t border-[var(--app-border)] bg-[var(--app-surface)] space-y-3">
            {isDesktop && (
              <button
                ...
                className="w-full py-3 bg-[var(--app-surface)] border border-[var(--app-primary)] text-[var(--app-primary)] rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-[var(--app-primary)] hover:text-white transition-all"
              >
            ...
            <button
              ...
              disabled={selectedIds.length === 0}
              className="w-full py-3 bg-[var(--app-primary)] text-white rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg"
            >
              <Download size={18} /> Generate Script
            </button>
          </div>
        )}
      </div>
