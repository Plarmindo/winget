...
export const GitHubPanel: React.FC<GitHubPanelProps> = ({ token, query, onClone, onDirectInstall, onFetchDetails }) => {
  ...
  useEffect(() => {
    const mapRepos = async () => {
      ...
      const reposToMap =
        activeTab === 'repos'
          ...
          : starredRepoObjects.filter(
              (r) =>
                (r.name || '').toLowerCase().includes(safeQuery) ||
                (r.description || '').toLowerCase().includes(safeQuery)
            );
    ...
    mapRepos();
    // mapToPackage is recreated each render; adding it would re-run this effect on every render.
  ...
  const handleGitHubAction = async (id: string, action: GitHubAction) => {
    ...
    try {
      switch (action) {
        case 'star':
          if (starredRepos.has(id)) {
            ...
            toggleStar(id, true);
            showToast(`Unstarred ${repo}`, 'success');
          } else {
            await starRepo(owner, repo, token);
            toggleStar(id, false);
            showToast(`Starred ${repo}`, 'success');
          }
          break;
        case 'fork':
          await forkRepo(owner, repo, token);
          showToast(`Forked ${repo} to your account!`, 'success');
          loadData();
          break;
        case 'watch':
          if (watchedRepos.has(id)) {
            ...
            showToast(`Stopped watching ${repo}`, 'success');
          } else {
            await watchRepo(owner, repo, token);
            setWatchedRepos((prev) => new Set(prev).add(id));
            showToast(`Now watching ${repo}`, 'success');
          }
          break;
        case 'details':
          setDetailsModal({
      ...
      showToast(`Action failed: ${e instanceof Error ? e.message : String(e)}`, 'error');
    }
  };
  ...
  const handleCreateRepo = async () => {
    ...
    try {
      ...
      showToast(`Repository "${createModal.name}" created successfully!`, 'success');
      setCreateModal({ open: false, loading: false, name: '', description: '', isPrivate: false, autoInit: true });
      loadData();
    } catch (e: unknown) {
      showToast(`Failed to create repo: ${e instanceof Error ? e.message : String(e)}`, 'error');
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button
          onClick={() =>
            setCreateModal({ open: true, loading: false, name: '', description: '', isPrivate: false, autoInit: true })
          }
      ...
      {detailsModal.open && (
        ...
        >
          <div
            className="bg-[var(--app-surface)] border border-[var(--app-border)] rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col"
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-[var(--app-border)]">
              ...
              <button
                onClick={() => setDetailsModal((prev) => ({ ...prev, open: false }))}
                className="p-2 hover:bg-[var(--app-bg)] rounded-lg"
              >
                <X size={18} />
              </button>
            </div>
            ...
            {detailsModal.loading ? (
              ...
              <div className="p-4 overflow-y-auto space-y-4">
                ...
                {detailsModal.release && (
                  <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                    <h3 className="font-semibold flex items-center gap-2 mb-2">
                      <Tag size={16} className="text-emerald-500" /> Latest Release
                    </h3>
                    <a
                      href={detailsModal.release.html_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[var(--app-primary)] hover:underline font-mono text-sm"
                    >
                ...
                <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                  <h3 className="font-semibold flex items-center gap-2 mb-2">
                    <GitBranch size={16} className="text-purple-500" /> Branches ({detailsModal.branches.length})
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {detailsModal.branches.slice(0, 10).map((b) => (
                      <span key={b.name} className="px-2 py-1 bg-[var(--app-surface)] rounded text-xs font-mono">
                        {b.name}
                      </span>
                    ))}
                    {detailsModal.branches.length > 10 && (
                      <span className="px-2 py-1 text-xs text-[var(--app-text-muted)]">
                        +{detailsModal.branches.length - 10} more
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                  <h3 className="font-semibold flex items-center gap-2 mb-2">
                    <GitCommit size={16} className="text-blue-500" /> Recent Commits
                  </h3>
                  <div className="space-y-2">
                    {detailsModal.commits.map((c) => (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block p-2 hover:bg-[var(--app-surface)] rounded text-sm"
                      >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                    <h3 className="font-semibold flex items-center gap-2 mb-2">
                      <AlertTriangle size={16} className="text-amber-500" /> Open Issues ({detailsModal.issues.length})
                    </h3>
                    <div className="space-y-1">
                      {detailsModal.issues.slice(0, 5).map((i) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="block text-xs truncate hover:text-[var(--app-primary)]"
                        >
                          #{i.number} {i.title}
                        </a>
                      ))}
                      {detailsModal.issues.length === 0 && (
                        <span className="text-xs text-[var(--app-text-muted)]">No open issues</span>
                      )}
                    </div>
                  </div>
                  <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                    <h3 className="font-semibold flex items-center gap-2 mb-2">
                      <GitPullRequest size={16} className="text-green-500" /> Open PRs ({detailsModal.prs.length})
                    </h3>
                    <div className="space-y-1">
                      ...
                      {detailsModal.prs.length === 0 && (
                        <span className="text-xs text-[var(--app-text-muted)]">No open PRs</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {detailsModal.readme && (
                  <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                    <h3 className="font-semibold flex items-center gap-2 mb-2">
                      <FileText size={16} className="text-blue-400" /> README
                    </h3>
                ...
                <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                  <h3 className="font-semibold flex items-center gap-2 mb-2">
                    ...
                    {detailsModal.currentPath && (
                      <button
                        onClick={() => navigateToFolder(detailsModal.currentPath.split('/').slice(0, -1).join('/'))}
                        className="text-xs text-[var(--app-primary)] ml-2"
                      >
                        ← Back
                      </button>
                    )}
                    ...
                    {detailsModal.contents.slice(0, 20).map((item) => (
                      ...
                      >
                        {item.type === 'dir' ? (
                          ...
                          <File size={14} className="text-[var(--app-text-muted)]" />
                        )}
                ...
                <div className="bg-[var(--app-bg)] rounded-lg p-3 border border-[var(--app-border)]">
                  <h3 className="font-semibold flex items-center gap-2 mb-2">
                    <Copy size={16} className="text-purple-500" /> Clone URLs
                  </h3>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-[var(--app-text-muted)] w-12">HTTPS</span>
                      <code className="flex-1 text-xs bg-[var(--app-surface)] px-2 py-1 rounded font-mono truncate">
                        https://github.com/{detailsModal.repoId}.git
                      </code>{' '}
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(`https://github.com/${detailsModal.repoId}.git`);
                          showToast('Copied!', 'success');
                        }}
                        className="text-xs text-[var(--app-primary)]"
                      >
                        <Copy size={12} />
                      </button>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <code className="flex-1 text-xs bg-[var(--app-surface)] px-2 py-1 rounded font-mono truncate">
                        git@github.com:{detailsModal.repoId}.git
                      </code>{' '}
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(`git@github.com:${detailsModal.repoId}.git`);
                          showToast('Copied!', 'success');
                        }}
                        className="text-xs text-[var(--app-primary)]"
                      >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <code className="flex-1 text-xs bg-[var(--app-surface)] px-2 py-1 rounded font-mono truncate">
                        gh repo clone {detailsModal.repoId}
                      </code>{' '}
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(`gh repo clone ${detailsModal.repoId}`);
                          showToast('Copied!', 'success');
                        }}
                        className="text-xs text-[var(--app-primary)]"
                      >
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    onClick={() => openUrl(`https://github.com/${detailsModal.repoId}`)}
                    className="flex-1 py-2 bg-[var(--app-primary)] text-white rounded-lg text-center font-medium hover:opacity-90"
                  >
                    View on GitHub
                  </button>
                  <button
                    onClick={() => openUrl(`https://github.com/${detailsModal.repoId}/issues`)}
                    className="flex-1 py-2 bg-[var(--app-bg)] border border-[var(--app-border)] rounded-lg text-center font-medium hover:bg-[var(--app-surface)]"
                  >
                    Issues
                  </button>
                  <button
                    onClick={() => openUrl(`https://github.com/${detailsModal.repoId}/pulls`)}
                    className="flex-1 py-2 bg-[var(--app-bg)] border border-[var(--app-border)] rounded-lg text-center font-medium hover:bg-[var(--app-surface)]"
                  >
                    PRs
                  </button>
      ...
      {createModal.open && (
        ...
        >
          <div
            className="bg-[var(--app-surface)] border border-[var(--app-border)] rounded-2xl w-full max-w-md overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 border-b border-[var(--app-border)]">
              <h2 className="font-bold text-lg flex items-center gap-2">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={createModal.isPrivate}
                    onChange={(e) => setCreateModal((prev) => ({ ...prev, isPrivate: e.target.checked }))}
                    className="rounded"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={createModal.autoInit}
                    onChange={(e) => setCreateModal((prev) => ({ ...prev, autoInit: e.target.checked }))}
                    className="rounded"
                  />
