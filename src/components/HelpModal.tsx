...
import { X, Search, Download, Terminal, Accessibility, PlayCircle, MousePointer, ChevronRight, HelpCircle, Image } from 'lucide-react';
...
export const HelpModal: React.FC<HelpModalProps> = ({ isOpen, onClose }) => {
    ...
    },
    {
      ...
      steps: [
        ...
        },
        {
          ...
          visualType: 'cart',
          videoUrl: '/help/select_packages.png'
        }
      ]
    },
    {
  ...
  // A component to display the visual aid (Image/GIF/Video)
  const VisualPlaceholder = ({ step }: { step: HelpStep }) => {
    if (step.videoUrl) {
      const isVideo = step.videoUrl.endsWith('.webm') || step.videoUrl.endsWith('.mp4');
      return (
        <div className="w-full h-48 rounded-xl border border-[var(--app-border)] overflow-hidden mb-4 bg-black/5 relative group">
          {isVideo ? (
            <video
              src={step.videoUrl}
              autoPlay
          ...
          <div className="absolute bottom-2 right-2 text-[10px] bg-black/70 text-white px-2 py-1 rounded opacity-70">
            {isVideo ? 'VIDEO' : 'GIF'}
          </div>
        </div>
      );
