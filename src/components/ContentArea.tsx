...
export const ContentArea: React.FC<ContentAreaProps> = ({
  ...
  if (packages.length === 0 && !searched && !loading && mode !== 'github') {
    if (mode === 'install') {
      return <WelcomeScreen handleSearch={handleSearch} openSettings={openSettings} />;
    }
    return (
      <MaintenanceImport
      ...
      <div className="w-full mb-8">
        {searched && packages.length === 0 && !loading ? (
          <EmptyState
            ...
            description={
              query
                ? `No results for "${query}". Try different keywords or check your package manager settings.`
