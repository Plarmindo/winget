...
export const MaintenanceImport: React.FC<MaintenanceImportProps> = ({
  importText,
  setImportText,
  importError,
  handleImport,
  ...
  return (
    <div className="max-w-3xl mx-auto mt-8">
      <div
        className={`rounded-2xl border p-8 bg-[var(--app-surface)]/50 ${mode === 'upgrade' ? 'border-emerald-900/50' : 'border-red-900/50'}`}
      >
        <div className="text-center mb-8">
          <div
            className={`inline-flex p-4 rounded-full mb-4 bg-opacity-10 ${mode === 'upgrade' ? 'bg-emerald-500 text-emerald-500' : 'bg-red-500 text-red-500'}`}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="bg-[var(--app-bg)] p-4 rounded-lg border border-[var(--app-border)]">
              ...
              <span className="text-[var(--app-text-muted)]">
                Run{' '}
                <code className="bg-[var(--app-surface)] px-1 py-0.5 rounded text-[var(--app-primary)]">
                  {settings.activePackageManager} {mode === 'upgrade' ? 'upgrade' : 'list'}
                </code>
              </span>
            </div>
            <div className="bg-[var(--app-bg)] p-4 rounded-lg border border-[var(--app-border)]">
          ...
          <div className="relative">
            <textarea
              ...
              className={`w-full h-48 bg-[var(--app-bg)] border rounded-xl p-4 font-mono text-xs focus:ring-2 focus:outline-none transition-all text-[var(--app-text)] ${
                mode === 'upgrade'
                  ? 'border-emerald-900/30 focus:border-emerald-500 focus:ring-emerald-900/20'
