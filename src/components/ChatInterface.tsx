      ...
      {parts.map((part, index) => {
        ...
        if (part.trim().startsWith('|') && part.includes('---')) {
          ...
          return (
            <div
              key={index}
              className="overflow-hidden my-4 border border-[var(--app-border)] rounded-xl shadow-md bg-[var(--app-surface)]"
            >
                  ...
                  <thead>
                    <tr className="bg-[var(--app-primary)]/10 border-b border-[var(--app-border)]">
                      {header.map((h, i) => (
                        <th
                          key={i}
                          className={`p-4 font-bold text-[var(--app-text)] whitespace-nowrap ${i === 0 ? 'bg-[var(--app-surface)]/50' : ''}`}
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {body.map((row, rI) => (
                      <tr key={rI} className="hover:bg-[var(--app-primary)]/5 transition-colors group">
                        {row.map((c, cI) => (
                          <td
                            key={cI}
                            className={`p-3 align-top ${cI === 0 ? 'font-semibold text-[var(--app-text-muted)] bg-[var(--app-bg)]/30 border-r border-[var(--app-border)]' : 'text-[var(--app-text)]'}`}
                          >
                            {c}
                          </td>
                        ))}
            ...
            {part.split('```').map((chunk, i) => {
                    ...
                    if (Array.isArray(parsed) && parsed.length > 0 && parsed[0].id && parsed[0].name) {
                      return (
                        <div key={i} className="my-3 grid gap-2">
                          {parsed.map((pkg: WingetPackage) => (
                            <div
                              key={pkg.id}
                              className="flex items-center justify-between p-3 bg-[var(--app-surface)] border border-[var(--app-border)] rounded-lg hover:border-[var(--app-primary)] transition-colors"
                            >
              ...
              return (
                <span key={i}>
                  {boldParts.map((b, bi) =>
                    bi % 2 === 1 ? (
                      <strong key={bi} className="text-[var(--app-primary)] font-bold">
                        {b}
                      </strong>
                    ) : (
                      b
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  // A3: Updated to use unified token bucket rate limiter
  const { isRateLimited, secondsRemaining, checkRateLimit } = useRateLimit(10, 10 / 60); // 10 requests per minute
  ...
  useEffect(() => {
    ...
    // handleSend/onClearPendingMessage change identity each render; the effect is meant to run per pendingMessage.
  ...
  }, [settings.aiConfig.provider, settings.aiConfig.baseUrl, isOpen, ollamaModels.length]);
  ...
  const handleSend = async (textInput?: string) => {
    ...
    if (!checkRateLimit()) {
      addChatMessage({
        role: 'model',
        text: `You're sending messages too quickly. Please wait ${secondsRemaining} seconds.`,
      });
      return;
    }
    ...
    try {
      ...
      addChatMessage({
        role: 'model' as const,
        text: `Error connecting to AI: ${e instanceof Error ? e.message : String(e)}`,
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const getModelIcon = () => {
    switch (modelType) {
      ...
      case 'thinking':
        return <BrainCircuit size={14} className="text-blue-400" />;
      default:
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-40 p-4 bg-[var(--app-primary)] text-white rounded-full shadow-lg hover:scale-110 transition-transform"
      ...
      {isOpen && (
        <div
          className="fixed bottom-24 right-6 w-[450px] max-w-[calc(100vw-2rem)] h-[600px] max-h-[calc(100vh-8rem)] bg-[var(--app-surface)] border border-[var(--app-border)] rounded-2xl shadow-2xl flex flex-col overflow-hidden z-40 animate-in slide-in-from-bottom-5 fade-in duration-300"
          data-testid="chat-interface"
        >
          {/* Header */}
          <div className="p-3 border-b border-[var(--app-border)] bg-[var(--app-bg)] flex justify-between items-center shrink-0">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-[var(--app-primary)]/20 rounded-lg text-[var(--app-primary)]">
                <Bot size={18} />
              </div>
              <div>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="relative group max-w-[200px]">
                  ...
                  >
                    ...
                    {settings.aiConfig.provider === 'ollama' && (
                      <>
                        {ollamaModels.length > 0 ? (
                          ...
                          <option value={settings.aiConfig.modelId || ''}>
                            {settings.aiConfig.modelId || 'Loading...'}
                          </option>
                        )}
                      </>
                    )}
                    {settings.aiConfig.provider === 'custom' && (
                      <option value={settings.aiConfig.modelId || ''}>
                        {settings.aiConfig.modelId || 'Custom Model'}
                      </option>
                    )}
                  </select>
                  {/* Custom Arrow */}
              ...
              <button
                ...
                className="p-1.5 hover:bg-red-500/10 rounded-lg text-[var(--app-text-muted)] hover:text-red-500 transition-colors"
                title="Clear Chat"
              >
                <Trash2 size={16} />
              </button>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[var(--app-bg)]/50 scroll-smooth">
            ...
            {chatMessages.map((m) => (
              <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-[90%] p-3 rounded-2xl text-sm shadow-sm ${m.role === 'user' ? 'bg-[var(--app-primary)] text-white rounded-tr-none user-message-bubble' : 'bg-[var(--app-surface)] border border-[var(--app-border)] rounded-tl-none'}`}
                  data-testid={m.role === 'model' ? 'chat-response' : undefined}
                >
                        ...
                        {m.sources.map((s, i) => (
                          <li key={i}>
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="underline hover:text-[var(--app-primary)] truncate block max-w-[200px]"
                            >
                              {s.title}
                            </a>
                          </li>
          ...
          <div className="p-3 border-t border-[var(--app-border)] bg-[var(--app-surface)] shrink-0">
            <div className="flex gap-2">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  {isRateLimited ? (
                    <span className="text-xs font-mono w-4">{secondsRemaining}</span>
                  ) : (
                    <Send size={18} />
                  )}
                </button>
