...
import { generateAIResponse, generateComparisonPrompt, listUpgradablePackages, listInstalledPackages, parseWingetOutput } from '../services/wingetService';
...
export const useAppController = (
    ...
    useEffect(() => {
        ...
        const setup = async () => {
            ...
            unlistenFn = await listen<{ operation: string; percent: number; message: string }>('operation-progress', (event) => {
    ...
    }, [mode, query, handleSearch, setLoading, setPackages, storePackagesForFiltering, setError]);

    // Data Handlers
    const handleClearData = useCallback((type: 'cart' | 'chat' | 'all') => {
    ...
    const handleImport = useCallback((importText: string, setImportText: (s: string) => void, setImportError: (s: string | null) => void) => {
    ...
    const runComparison = useCallback(async () => {
        ...
        try {
            ...
            const result = await generateAIResponse(useAppStore.getState().settings, prompt, "You are a software comparison expert...", true);
            setCompareResult(result);
        } catch (e: any) {
            setCompareResult(`Failed to generate comparison: ${e.message}`);
        } finally {
