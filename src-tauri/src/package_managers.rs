...
pub fn run_scoop_search(query: &str) -> Result<String, String> {
    ...
    let output = output_result.map_err(|e| {
        if e.kind() == std::io::ErrorKind::NotFound {
            "Scoop is not installed or not in PATH. Please install Scoop to use this provider."
                .to_string()
        } else {
...
pub fn run_manager_operation(
    ...
    // Validate Input
    if let Err(e) = crate::validation::validate_package_id(package_id) {
    ...
    let output = output_result.map_err(|e| format!("Failed to start {}: {}", manager, e))?;
